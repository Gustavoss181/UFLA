...
struct Registro{
	...
	friend ostream & operator << (ostream &out, const Registro &c);
};

ostream & operator << (ostream &out, const Registro &r){
    out << r.s1 << endl;
    return out;
}
...
void createInitialRuns(int run_size, int num_ways){
	...
	Registro* arr = new Registro[run_size];
...
int main(){
	...
	if(bin){
		...
		for (int i = 0; i < num_ways * run_size; i++){
			Registro aux;
			//aux.n1 = rand()%100;
			for(int i=0; i<sizeof(aux.s1)-5; i++)
				aux.s1[i] = (char) (rand()%26 + 97);
			bin.write((const char *)(&aux), sizeof(Registro));
		}
		bin.close();
	}
