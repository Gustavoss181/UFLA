/* 
    ...
    Solução de: Gustavo Soares Silva
*/
...
class estoque {
    ...
    public:
        ...
        estoque(const estoque &est);//construtor de cópia
        ~estoque();
...
estoque::estoque(const estoque &est){
    quantidadeDeProdutos = est.quantidadeDeProdutos;
    capacidadeMaxima = est.capacidadeMaxima;
    listaDeProdutos = new produto[capacidadeMaxima];
    for(int i=0; i<quantidadeDeProdutos; i++)
        listaDeProdutos[i] = est.listaDeProdutos[i];
}
...
void estoque::redimensionarCapacidade() {
    ...
    int aumentar = 1 + capacidadeMaxima;
	produto *novo = new produto[aumentar];
    
	//cópia dos dados pro novo vetor
	for(int i=0; i<quantidadeDeProdutos; i++){
		novo[i] = listaDeProdutos[i];
    }

	capacidadeMaxima = aumentar;
	delete [] listaDeProdutos;
	listaDeProdutos = novo;
}
